...
<div class="container d-flex justify-content-center mt-3">
    ...
    <div class="row">
        ...
        <div class="col-md-2 d-flex align-items-center justify-content-end">
            <a href="{{ route('favorites.destroy',$favorite_product->id)}}" class="samuraimart-favorite-item-delete" onclick="event.preventDefault();document.getElementById('favorite-destroy-form{{$favorite_product->id}}').submit();">
                削除
            </a>
            <form id="favorite-destroy-form{{$favorite_product->id}}" action="{{route('favorites.destroy',$favorite_product->id)}}" method="POST" class="d-none">
                @csrf
                @method('delete')
            </form>
        </div>
        <div class="col-md-3 d-flex align-items-center justify-content-end">
            <form action="{{route('carts.store')}}" class="m-3 align-items-end" method="post">
                @csrf
